...
export default function ShiningButton({ children, onClick }: Props) {
  return (
    ...
    >
      <span className="-skew-x-20 absolute top-0 left-[-100%] h-full w-full animate-shine bg-gradient-to-r from-transparent via-black/10 to-transparent opacity-0 dark:via-white/40" />
      {children}
    </button>
  );
}
